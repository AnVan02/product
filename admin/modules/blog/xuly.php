...
$data = $_GET ['data']  ?? null;
$article_ids = json_decode ($data , true);
...
if (isset($_POST['article_add'])) {
    ...
    if (isset($_POST ['article_add'])){
        if (empty ($article_id)){
            $article_id = uniqid ('aricle_');
        }
        $sql_check = "SELECT * FROM article WHERE article_id = ?";
        $stmt = $mysqli -> prepage ($sql_check);
        if(!$stmt) die("lỗi SQL: ".$mysqli -> error);
        $stmt -> bind_param ("s",article_link);
        $stmt -> execute();
        $result -> $stmt -> get_result();
        ...
        if($result -> num_rows > 0) {
            die ("<script>alert('Lỗi: ID đường dẫn đã tồn tại !'); window.history.back();</script>");
        }
    }
    // Upload ảnh nếu có
    if ($article_image && $article_image_tmp) {
        move_uploaded_file($article_image_tmp, 'modules/blog/uploads/' . $article_image);
    }
    ...
    // Thêm vào database
    $sql_add = "INSERT INTO article (article_id, article_link, article_author, article_title, article_summary, article_content, article_image, article_date, article_tag, article_status, article_video) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $mysqli->prepare($sql_add);
    if (!$stmt) die("Lỗi SQL: " . $mysqli->error);
    $stmt->bind_param("sssssssssss",$article_id, $article_link, $article_author, $article_title, $full_summary, $article_content, $article_image, $article_date, $article_tag, $article_status, $videoName);
...
// Sửa bài viết
    ...
    // Cập nhập bài viết 
    $sql_update = "UPDATE article SET article_author=?, article_title=?, article_summary=?, article_content=?, article_image=?, article_date=?, article_tag=?, article_status=?, articel_link WHERE article_id=?";
    $stmt = $mysqli->prepare($sql_update);
    if (!$stmt) die("Lỗi SQL: " . $mysqli->error);
    $stmt->bind_param("ssssssssss", $article_author, $article_title, $article_summary, $article_content, $article_image, $article_date, $article_tag, $article_status, $article_link, $article_id);
    if (!$stmt->execute()) {
