...
<form method="POST" action="modules/blog/xuly.php" enctype="multipart/form-data">
                ...
                <div class="card-body">
                    <div class="card-content">
                       ...
                       <div class="form-group">
                            ...
                            <input type="date" name="article_date" id="article_date" class="form-control" value="<?php echo date('Y-m-d'); ?>"> <!-- Giá trị mặc định là ngày hiện tại -->
                        </div>
                        ...
                        <div class="input-item form-group">
                            <label for="article_content" class="d-block">Tóm tắt bài viết</label>
    ...
    //     contentsCss: 'https://cdn.ckeditor.com/4.16.2/standard-all/contents.css'
    //     uploads ='../moduler/blog/upload'
    // });
...
$(document).ready(function() {
    $('#article_content').summernote({
        ...
        callbacks: {
            onMediaDelete: function(target) {
                // Gửi Ajax xoá file trên server nếu cần
                console.log("Media removed: ", target[0].src);
            },
            onImageUpload: function(files) {
                uploadMedia(files[0], '#article_content');
            },
    ...
    $('#article_summary').summernote({
        ...
        callbacks: {
            onImageUpload: function(files) {
                uploadMedia(files[0], '#article_summary');
            },
            onVideoUpload: function(files) {
                uploadMedia(files[0], '#article_summary');
            }
    ...
    function uploadMedia(file, editorSelector) {
        let data = new FormData();
        data.append("file", file);
        $.ajax({
            url: 'modules/blog/upload_summernote.php', // Bạn sẽ tạo file này
            method: "POST",
            data: data,
            processData: false,
            contentType: false,
            success: function(url) {
                if (file.type.startsWith("video/")) {
                    $(editorSelector).summernote('editor.insertNode', $('<video controls><source src="' + url + '" type="' + file.type + '"></video>')[0]);
                } else {
                    $(editorSelector).summernote('insertImage', url);
                }
            },
